...
Nodo* get_most_right(Nodo* parent){
  	while (!parent->isLeaf){
		if(parent->m_pSon[2]) parent = parent->m_pSon[2];
		else parent = parent->m_pSon[0];
    }
  	return parent;
}
...
Nodo* get_most_left(Nodo* parent){
	while (!parent->isLeaf){
		if(parent->m_pSon[0]) parent = parent->m_pSon[0];
		else parent = parent->m_pSon[2];
    }
  	return parent;
}
...
Nodo* create_range_tree(vector <pii> v, int l, int h, bool axis=true) {
	...
	Nodo * child_left_y = nullptr;
	Nodo * child_right_y = nullptr;

	if (axis) {
		child_left_y = create_range_tree(v,l, m, false);
		child_left_x->m_pSon[1] = child_left_y;
		child_right_y = create_range_tree(v,m+1, h, false);
