...
Nodo* create_range_tree(vector <pii> v, int l, int h, bool axis=true) {
    ...
    sort(v.begin()+l, v.begin()+h, comparex);
    Nodo* parent_left_x = create_range_tree(v,l, m, axis);
    Nodo* parent_right_x = create_range_tree(v,m+1, h, axis);
    
    sort(v.begin()+l, v.begin()+h, comparey);
    Nodo* parent_left_y = create_range_tree(v,l, m, false);
    parent_left_x->m_pSon[1] = parent_left_y;
    Nodo* parent_right_y = create_range_tree(v,m+1, h, false);
    parent_right_x->m_pSon[1] = parent_right_y;

     
    Nodo* parent_x = new Nodo((parent_left_x->x + parent_right_x->x)/2, false);
    parent_x->m_pSon[0] = parent_left_x;
    Nodo* parent_y = new Nodo((parent_left_y->x + parent_right_y->x)/2, false);
    parent_y->m_pSon[0] = parent_left_y;
    parent_y->m_pSon[2] = parent_right_y;

    parent_x->m_pSon[1] = parent_y;
    parent_x->m_pSon[2] = parent_right_x;
